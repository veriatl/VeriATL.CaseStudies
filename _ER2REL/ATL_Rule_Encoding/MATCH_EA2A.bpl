...
rule EA2A { 
from 
	att : ER!ERAttribute, ent : ER!Entity (att.entity = ent)
to t : REL!RELAttribute 
	( name <- att.name, isKey <- att.isKey ) 
}
 */
...
procedure EA2A_matchAllTest() returns ();
...
implementation EA2A_matchAllTest() returns ()
{

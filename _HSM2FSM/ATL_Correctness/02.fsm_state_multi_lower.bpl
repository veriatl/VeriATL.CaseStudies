procedure fsm_state_multi_lower()
...
ensures (forall st: ref:: {read($tarHeap,st,alloc),st!=null,dtype(st)<:FSM$AbstractState}
	...
	==>
	read($tarHeap,st,FSM$AbstractState.stateMachine) != null && read($tarHeap,read($tarHeap,st,FSM$AbstractState.stateMachine),alloc)
);
{

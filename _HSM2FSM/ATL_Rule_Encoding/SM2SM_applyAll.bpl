...
assert dtype($Unbox(Seq#Index(stk, Seq#Length(stk)-1))) <: HSM$StateMachine;
stk := Seq#Build(Seq#Take(stk, Seq#Length(stk)-1), $Box(read($srcHeap,$Unbox(Seq#Index(stk, Seq#Length(stk)-1)),FieldOfDecl(dtype($Unbox(Seq#Index(stk, Seq#Length(stk)-1))), _Field$name): Field (String))));
...
assert dtype($Unbox(Seq#Index(stk, Seq#Length(stk)-2))) <: FSM$StateMachine;

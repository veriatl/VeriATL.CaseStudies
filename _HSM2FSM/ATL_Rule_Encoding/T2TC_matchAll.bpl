 ...
 invariant (forall i: int:: 0<=i &&i <$i ==>
  true ==>(
    Seq#Index(obj#4,i)!=null && read($srcHeap, Seq#Index(obj#4,i), alloc) && dtype(Seq#Index(obj#4,i)) <: HSM$Transition ==> 
	  (forall __src, __trg, __c: ref :: Seq#Contains(Fun#LIB#AllInstanceFrom(old($srcHeap),HSM$AbstractState),__src) && Seq#Contains(Fun#LIB#AllInstanceFrom(old($srcHeap),HSM$CompositeState),__trg) && Seq#Contains(Fun#LIB#AllInstanceFrom(old($srcHeap),HSM$InitialState),__c) ==>		
		 read($srcHeap, Seq#Index(obj#4,i), HSM$Transition.source) == __src 
		 && read($srcHeap, Seq#Index(obj#4,i), HSM$Transition.target) == __trg 
		 && read($srcHeap, __c, HSM$AbstractState.compositeState) == __trg
		 && !(dtype(__src) == HSM$CompositeState)  ==>
...
{ 
 ...
 invariant (forall _src: ref ::
	_src!=null && read($srcHeap, _src, alloc) && dtype(_src) <: HSM$AbstractState ==> Seq#Contains(obj#11,_src));
  invariant (forall j: int:: 0<=j &&j <$j ==>
  true ==>(
    Seq#Index(obj#11,j)!=null && read($srcHeap, Seq#Index(obj#11,j), alloc) && dtype(Seq#Index(obj#11,j)) <: HSM$AbstractState ==> 
	  (forall __trg, __c: ref :: Seq#Contains(Fun#LIB#AllInstanceFrom(old($srcHeap),HSM$CompositeState),__trg) && Seq#Contains(Fun#LIB#AllInstanceFrom(old($srcHeap),HSM$InitialState),__c) ==>		
		 read($srcHeap, Seq#Index(obj#4,$i), HSM$Transition.source) == Seq#Index(obj#11,j) 
		 && read($srcHeap, Seq#Index(obj#4,$i), HSM$Transition.target) == __trg 
		 && read($srcHeap, __c, HSM$AbstractState.compositeState) == __trg
		 && !(dtype(Seq#Index(obj#11,j)) == HSM$CompositeState)  ==>
 ...
 invariant (forall _trg: ref ::
	_trg!=null && read($srcHeap, _trg, alloc) && dtype(_trg) <: HSM$CompositeState ==> Seq#Contains(obj#18,_trg));
 invariant (forall k: int:: 0<=k &&k <$k ==>
  true ==>(
    Seq#Index(obj#18,k)!=null && read($srcHeap, Seq#Index(obj#18,k), alloc) && dtype(Seq#Index(obj#18,k)) <: HSM$CompositeState ==> 
	  (forall __c: ref ::  Seq#Contains(Fun#LIB#AllInstanceFrom(old($srcHeap),HSM$InitialState),__c) ==>		
		 read($srcHeap, Seq#Index(obj#4,$i), HSM$Transition.source) == Seq#Index(obj#11,$j) 
		 && read($srcHeap, Seq#Index(obj#4,$i), HSM$Transition.target) == Seq#Index(obj#18,k) 
		 && read($srcHeap, __c, HSM$AbstractState.compositeState) == Seq#Index(obj#18,k)
		 && !(dtype(Seq#Index(obj#11,$j)) == HSM$CompositeState)  ==>
 ...
 invariant (forall _c: ref ::
	_c!=null && read($srcHeap, _c, alloc) && dtype(_c) <: HSM$InitialState ==> Seq#Contains(obj#25,_c));
 invariant (forall l: int:: 0<=l &&l <$l ==>
  true ==>(
    Seq#Index(obj#25,l)!=null && read($srcHeap, Seq#Index(obj#25,l), alloc) && dtype(Seq#Index(obj#25,l)) <: HSM$InitialState ==> 
		 read($srcHeap, Seq#Index(obj#4,$i), HSM$Transition.source) == Seq#Index(obj#11,$j) 
		 && read($srcHeap, Seq#Index(obj#4,$i), HSM$Transition.target) == Seq#Index(obj#18,$k) 
		 && read($srcHeap, Seq#Index(obj#25,l), HSM$AbstractState.compositeState) == Seq#Index(obj#18,$k)
		 && !(dtype(Seq#Index(obj#11,$j)) == HSM$CompositeState)  ==>
  ...
  invariant (forall<alpha> $o : ref, $f: Field alpha ::
	...
	|| ((dtype($o) <: FSM$Transition) && Seq#Length(getTarsBySrcs_inverse($o)) == 4 && dtype(Seq#Index(getTarsBySrcs_inverse($o), 0)) <: HSM$Transition && dtype(Seq#Index(getTarsBySrcs_inverse($o), 1)) <: HSM$AbstractState && dtype(Seq#Index(getTarsBySrcs_inverse($o), 2)) <: HSM$CompositeState && dtype(Seq#Index(getTarsBySrcs_inverse($o), 3)) <: HSM$InitialState && $f==alloc )));				
			{ 
				...
				assert $Unbox(top(stk)) == (read($srcHeap, c, HSM$AbstractState.compositeState));
				call stk := OpCode#Load(stk, trg);
				call stk := OCLAny#Equal(stk,$Unbox(Seq#Index(stk, Seq#Length(stk)-2)):ref,$Unbox(Seq#Index(stk, Seq#Length(stk)-1)):ref);
